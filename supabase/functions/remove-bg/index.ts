...
serve(async (req) => {
  ...
  try {
    ...
    const prompt = "Remove the background from this image. Replace the removed background with a solid bright green color (exactly hex #00FF00). Keep the main subject intact with original colors, details, dimensions, and resolution — do NOT resize or crop. The output image must have the EXACT same pixel dimensions as the input. Return only the edited image with the green background, no text.";
